(() => {
    ...
    const fragmentSource = `
        ...
        vec3 spectral(float t) {
            ...
            float graphite = 0.035 + 0.62 * pow(wave, 1.65);
            return vec3(graphite);
        }
        ...
        void main() {
            ...
            vec2 p = uv * 2.0 - 1.0;
            float time = u_time * 0.30;
            float slowPulse = 0.5 + 0.5 * sin(time * 0.55);
            ...
            vec3 color = mix(vec3(0.002), vec3(0.075), haze);
            color += vec3(0.12) * deepGlow;

            vec2 starCell = floor(gl_FragCoord.xy / 3.0);
            float star = step(0.996, hash21(starCell));
            color += star * vec3(0.58) * (0.32 + 0.38 * sin(time * 1.2 + hash21(starCell) * 12.0));
            ...
            width *= (0.94 + 0.06 * sin(p.y * 15.0 - time * 1.4)) * (0.96 + 0.04 * slowPulse);
            ...
            snakeColor = mix(snakeColor, vec3(0.025), scales * 0.34);
            snakeColor += vec3(0.68) * edge * (0.34 + 0.18 * slowPulse);
            ...
            color += spectral(flow + 0.18) * aura * (0.09 + 0.08 * slowPulse);
            ...
            headColor += vec3(0.20) * fractalScales(headP * 2.4) * 0.32;
